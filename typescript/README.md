# TypeScript — API Testing Exercise

Test the [Deck of Cards API](https://deckofcardsapi.com/) through the provided `DeckOfCardsApiClient`.

> New here? Read the [repository overview](../README.md) first — it explains the exercise, the ground rules, and what we're evaluating. This file only covers the TypeScript-specific setup.

**Prerequisites:** Node.js 18+ (uses the built-in `fetch`), internet access
...
npm run lint        # eslint
npm run build       # emit dist/ (library build)
...
```
typescript/
├── src/                      # Library under test — do not modify
│   ├── client.ts             # DeckOfCardsApiClient (fetch-based)
│   ├── models.ts             # Typed responses + JSON parsers
│   └── index.ts              # Public entry point
├── tests/
│   ├── fixtures.ts           # Shared client instance
│   ├── examples/             # 3 worked examples (passing)
│   └── exercises/            # 4 candidate exercises (skipped)
├── package.json
├── tsconfig.json             # Type-checking config (src + tests)
├── tsconfig.build.json       # Library build config (src only)
├── vitest.config.ts
└── eslint.config.js
```
...
Implement the skipped tests in `tests/exercises/candidate-exercises.test.ts`. Switch each `it.skip` (and `it.skip.each`) to `it` / `it.each` when done.

Copy patterns from `tests/examples/deck-lifecycle.test.ts`. Use `DeckOfCardsApiClient` — read its methods and the API docs.
...
| 3   | Multi-deck card count (`it.each`, `{ deckCount: 2, expectedCards: 104 }`) | 2 decks contain 104 cards                                   |
...
- **Exercise 3 is data-driven** (`it.each`), not a plain `it`.
